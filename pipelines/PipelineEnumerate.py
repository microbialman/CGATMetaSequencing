...
def countFeatures(feature,gtf,pairedness,output,inputs,params):
    if params["General_count_method"=="featureCounts"]:
    ...
    if params["General_count_method"=="HTSeq"]:
        cfcall=["htseq-count -f bam"]
        if pairedness == True:
           cfcall.append("-r pos")
        cfcall.append("-s no -t {} -i {} --nonunique=all {} {} >> {}".format(params["featureCounts_t"],feature," ".join(inputs),gtf,output))
